...
# 확인문제1. 2018/01 매출의 총합
SELECT SUM(`sales`) FROM `SALE`
WHERE `year`=2018 AND `month`=1;
# 확인문제2. 2019/02 5만원 이상 매출에대한 총합과 평균을 구하시오.
SELECT 
	SUM(`sales`) AS `총합`,
	AVG(`sales`) AS `평균`
FROM 
...
SELECT `year`, SUM(`sales`) AS `합계` FROM `SALE` GROUP BY `year`;
